...
exports.albumSlider = albumSlider;
...
function albumSlider() {
  var albumSliderThumbs = new Swiper('.album-slider-thumbs', {
    spaceBetween: 30,
    slidesPerView: 3,
    watchSlidesVisibility: true
  });

  var albumSliderTop = new Swiper('.album-slider-top', {
    navigation: {
      nextEl: '.swiper-button-next',
      prevEl: '.swiper-button-prev'
    },
    thumbs: {
      swiper: albumSliderThumbs
    }
  });
}
...
var menuToggle = function menuToggle() {
  ...
  var menuItem = headerBlock.querySelector('.header__menu');
  var mobileBtn = headerBlock.querySelector('.mobile-btn');
...
var searchToggle = function searchToggle() {
  var searchBlock = [].concat(_toConsumableArray(document.querySelectorAll('.js-search')));

  searchBlock.forEach(function (el) {

    var searchBtn = el.querySelector('.icon-search');
    var searchBtnClose = el.querySelector('.icon-close');
    var searchField = el.querySelector('.search__field');
    var searchInput = el.querySelector('.search__input');

    searchBtn.addEventListener('click', function (e) {
      e.preventDefault();
      searchField.classList.add('is-active', 'animated', 'fadeIn');
      searchBtn.classList.add('is-hide');
      searchBtnClose.classList.remove('is-hide');
      searchInput.focus();
    });

    searchBtnClose.addEventListener('click', function (e) {
      searchField.classList.remove('is-active', 'animated', 'fadeIn');
      searchBtnClose.classList.add('is-hide');
      searchBtn.classList.remove('is-hide');
    });

    сloseSearchPanel(searchField, searchBtn, searchBtnClose);
  });
};

var сloseSearchPanel = function сloseSearchPanel(panel, btnOpen, btnClose) {
  var nullHeight = 0;

  document.addEventListener('mouseup', function (e) {
    if (!$(panel).is(e.target) && nullHeight === $(panel).has(e.target).length && $(panel).hasClass('is-active')) {
      $(panel).removeClass('is-active');
      btnClose.classList.add('is-hide');
      btnOpen.classList.remove('is-hide');
    }
  });
};
...
var languagePanel = function languagePanel() {
  var languageBlock = [].concat(_toConsumableArray(document.querySelectorAll('.language')));

  languageBlock.forEach(function (el) {
    drpoDown(el);
  });

  function drpoDown(el) {
    var langBtn = el.querySelector('.language__btn');
    var contentItem = el.querySelector('.language__content');
    var allLanguages = [].concat(_toConsumableArray(contentItem.children));

    el.addEventListener('click', function (e) {
      contentItem.classList.toggle('is-active');
    });
    ...
    allLanguages.forEach(function (el) {
      el.addEventListener('click', function (e) {
        if (e.target) {
          langBtn.innerHTML = e.target.innerHTML.slice(0, 2);
        }
      });
    });
  }
...
function mobileMenu() {
  var mobileMenu = document.querySelector('.js-mob-menu');
  var mobileMenuItems = [].concat(_toConsumableArray(mobileMenu.children));

  mobileMenuItems.forEach(function (el) {
    if (el.classList.contains('has-submenu')) {
      var clone = el.cloneNode(false);

      clone.innerHTML = el.querySelector('.arrow').innerHTML;

      clone.classList.remove('has-submenu');
      clone.classList.add('back');

      openSubmenu(el, clone);
    }
  });

  function openSubmenu(element, clone) {
    element.addEventListener('click', function (e) {

      element.querySelector('.nav--sub').classList.add('is-active');
      element.querySelector('.nav--sub').insertBefore(clone, element.querySelector('.nav--sub').firstChild);
  ...
  function back(element) {
    var back = mobileMenu.querySelector('.back');
    back.addEventListener('click', function (e) {
      event.stopPropagation();
      element.querySelector('.nav--sub').classList.remove('is-active');
    });
  }
}
...
$.validator.addMethod("eitherEmailPhone", function (value, element) {
  var isPhone = (this.optional(element) || /^\+|\d+$/.test(value)) && this.getLength($.trim(value), element) <= 15;
  var isEmail = this.optional(element) || /^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))$/i.test(value);
...
$.validator.addMethod("alpha", function (value, element) {
  return this.optional(element) || value == value.match(/^[a-zA-Zа-яА-Я0-9-ЯёЁІіЇїЄєҐґ'-_\. ]+$/);
});
...
$.validator.addMethod("names", function (value, element) {
  return this.optional(element) || value == value.match(/^['\x2DA-Za-z\u0401\u0404\u0406\u0407\u0410-\u044F\u0451\u0454\u0456\u0457\u0490\u0491][ '\x2D\.0-9A-Z_a-z\u0410-\u044F]{1,35}$/);
});
...
var answerHandler = function answerHandler(result, link) {
  var id = $(".js-ajax-popup");

  if ("undefined" != typeof result.total) {
    result.total.forEach(function (element) {
      $(".form__label").each(function (i, el) {
        $(el).find("strong").html(element);
      });
    });
  }
...
$("span[data-text]").click(function () {
  var $title = $(this).find(".form__label-title");

  if (!$title.length) {
    var span = $("<span class=\"form__label-title\">\n                    " + $(this).attr("data-text") + "\n                  </span>");
    $(this).append(span);
  } else {
    $title.remove();
  }
        ...
        $.ajax({
          ...
          error: function error(result) {
            ...
            if (errors.non_field_errors != undefined) {
              $form.find('label[data-nonfielderror="error"]').html(errors.non_field_errors);
            } else {
              for (field in errors) {
...
var dropdownFooter = function dropdownFooter() {
  var dropdownItem = [].concat(_toConsumableArray(document.querySelectorAll('.js-dropdown-footer')));
  ...
  function closeDropdown() {
    dropdownItem.forEach(function (el) {
      ...
      if (hide.classList.contains('is-hide')) {
        return;
      } else {
        hide.classList.add('is-hide');
      }
    });
  }
};
...
function initMap(el) {
  var address = document.querySelector('.js-map').dataset.address;

  var selector = el;
  var data = JSON.parse(el.dataset.coord);
  var center = { lat: data.lat, lng: data.lng };
  var scrollwheel = false;
  var offset = window.innerWidth < 1200 ? 0 : 1;
  var map = new google.maps.Map(selector, {
    scrollwheel: scrollwheel,

    center: { lat: data.lat, lng: data.lng + offset },
    zoom: 9,
    styles: [{
      featureType: "water",
...
function tabs() {
  var bindAll = function bindAll() {
    var menuElements = [].concat(_toConsumableArray(document.querySelectorAll('[data-point-number]')));
  ...
  var clear = function clear() {
    ...
    menuElements.forEach(function (el) {
      ...
      var id = el.getAttribute('data-point-number');
      document.querySelector('[data-info-number=\'' + id + '\']').classList.remove('is-active', 'animated', 'fadeIn');
    });
  };

  var change = function change(e) {
    dotsColored(e.target);
    line(e.target);
    clear();
    e.target.closest('.timeline__elem', 'is-active').classList.add('is-active');
    var id = e.currentTarget.getAttribute('data-point-number');
    document.querySelector('[data-info-number=\'' + id + '\']').classList.add('is-active', 'animated', 'fadeIn');
  };

  var dotsColored = function dotsColored(elem) {
    var test = elem.closest('.timeline__elem', 'is-active');
    var test2 = [].concat(_toConsumableArray(document.querySelectorAll('.timeline__elem')));
    test2.forEach(function (el) {
      el.classList.remove('previous');
    });
    $(test).prevAll().addClass('previous');
  };

  var line = function line(elem) {
    var timeLine = document.querySelector('.timeline__color-line');
    var targetElem = elem.closest('.timeline__elem', '.is-active');


    var size = targetElem.getBoundingClientRect().left + targetElem.getBoundingClientRect().width / 2;
    timeLine.style.width = size + 'px';
  };

  dotsColored(document.querySelector('.timeline__elem.is-active'));
  line(document.querySelector('.timeline__elem.is-active'));
  bindAll();
}
...
if (document.querySelector('.timeline')) {
  ...
  window.onload = function () {
    var elem = document.querySelector(".js-scroll"),
        scroll = 0;
...
var _sliderOption = __webpack_require__(133);

function _toConsumableArray(arr) { if (Array.isArray(arr)) { for (var i = 0, arr2 = Array(arr.length); i < arr.length; i++) { arr2[i] = arr[i]; } return arr2; } else { return Array.from(arr); } }

var openAlbumPopup = function openAlbumPopup() {

  var imageLinks = [].concat(_toConsumableArray(document.querySelectorAll('.js-link-album-popup')));
  var popup = document.querySelector('.album-popup');
  ...
  imageLinks.forEach(function (el, index) {
    el.addEventListener('click', function () {
      ...
      var albumSliderTop = new Swiper('.album-slider-top', {
        navigation: {
          nextEl: '.swiper-album-next',
          prevEl: '.swiper-album-prev'
        },
        thumbs: {
          swiper: albumSliderThumbs
        }
      ...
      albumSliderTop.slideTo(index, 1000, false);
    });
  });
...
if (document.querySelector('.album') || document.querySelector('.gallery-image')) {
  openAlbumPopup();
}
...
function animatedBlock() {
  ...
  var animation_elements = [].concat(_toConsumableArray(document.querySelectorAll('.js-animate')));

  function check_if_in_view() {

    var windowHeight = innerHeight / 2;

    animation_elements.forEach(function (el) {

      var itemHeight = el.getBoundingClientRect().top;

      if (itemHeight < windowHeight) {
        var animateType = el.dataset.typeAnimate;

        el.classList.add('animated');
        el.classList.add(animateType);

        if (el.querySelectorAll('.js-animate-card')) {
          var cards = [].concat(_toConsumableArray(el.querySelectorAll('.js-animate-card')));
          cards.forEach(function (elem) {
            var cardTypeAnimate = elem.dataset.typeAnimate;
            elem.classList.add('animated', '' + cardTypeAnimate);
            elem.classList.add(cardTypeAnimate);
          });
        }
      } else {}
    });
